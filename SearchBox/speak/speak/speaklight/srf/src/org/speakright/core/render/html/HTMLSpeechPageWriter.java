...
//			pw.renderSubmitBlock("http://abc.com");
	...
	{
		o(String.format("TRANSFER(%s) to '%s' with prompt '%s'<p>", xfer.m_transferType, xfer.m_destination, xfer.m_prompt.ptext()));
	}
	void renderRawContent(RawContent content)
	{
		o(String.format("RAW CONTENT")); //later show content when can safely escape it!!
	}
	void renderGotoUrl(GotoUrl gotoUrl)
	{
		o(String.format("GOTO-URL: %s", gotoUrl.getUrl()));
	}
	void renderRecordAudio(RecordAudio record)
	{
		o(String.format("RECORD-AUDIO: beep=%s max=%s finalSilence=%s", record.getBeep(), record.getMaxTime(),
				record.getFinalSilence()));
		PromptWriter pw = new PromptWriter(this);
		pw.renderSubmitBlock(m_rcontext.m_returnUrl);
	}
	public void endPage()
